...
def status(config: Path | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "font": value.display.font,
        "font_size": value.display.font_size,
        "sources": [
...
def write(text: str, config: Path | None = None, font: str | None = None) -> bool:
    """Write text directly, optionally overriding the configured font for this call."""
    display_config = _direct_display_config(config)
    if font is not None:
        if not font.strip():
            raise ValueError("font must not be empty")
        display_config = replace(display_config, font=font)
    display = build_display(display_config)
    try:
